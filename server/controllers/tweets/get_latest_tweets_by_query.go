...
func GetLatestTweetsByQuery(ctx *fasthttp.RequestCtx) {
  ...
  `, query, limit, page)
	...
	if err != nil {
		response := responses.CreateErrorResponse(&responses.Error{
			...
			Message: "Failed to load latest Tweets",
		})

		go logging.Logger.Error("Failed to load latest Tweets", zap.Error(err))
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			response := responses.CreateErrorResponse(&responses.Error{
				Status:  fasthttp.StatusInternalServerError,
				Message: "Failed to decode latest Tweets",
			})

			go logging.Logger.Error("Failed to decode latest Tweets", zap.Error(err))
			ctx.SetStatusCode(fasthttp.StatusInternalServerError)
			ctx.Write(response)
			return
		}
