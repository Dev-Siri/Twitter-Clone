...
func GetMediaTweetsByQuery(ctx *fasthttp.RequestCtx) {
  ...
  `, query, limit, page)
	...
	if err != nil {
		...
		go logging.Logger.Error("Failed to load media Tweets", zap.Error(err))
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			response := responses.CreateErrorResponse(&responses.Error{
				Status:  fasthttp.StatusInternalServerError,
				Message: "Failed to decode media Tweets",
			})

			go logging.Logger.Error("Failed to decode media Tweets", zap.Error(err))
			ctx.SetStatusCode(fasthttp.StatusInternalServerError)
			ctx.Write(response)
			return
		}
