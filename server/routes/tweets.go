...
func RegisterTweetsRoutes(r *router.Router) {
	...
	r.GET("/tweets/search", tweet_controllers.GetTweetsByQuery)
	r.GET("/tweets/search/latest", tweet_controllers.GetLatestTweetsByQuery)
	r.GET("/tweets/search/media", tweet_controllers.GetMediaTweetsByQuery)
